...
Unicode_bt ='utf-8' #中文字符编码方式，我的机器上是gb2312，服务器上是utf-8，还有个别机器上是GBK
...
def first_locate():
    ...
    for paragr in _iter(xml_tree,'p'):
        ...
        """
        print current_part
        if current_part == 'menu' and not menuEnd:
            print paraNum
            count = 0
            for fldchar in _iter(paragr,"fldChar"):
                if count == 0 and get_val(fldchar,"fldCharType") == "begin":
                    count += 1
                    continue
                if count == 1 and get_val(fldchar,"fldCharType") == 'seperate':
                    count += 1
                    continue
                if get_val(fldchar,"fldCharType") == 'end' and count == 0:
                    print count
                    menuEnd = 'true'
        """
